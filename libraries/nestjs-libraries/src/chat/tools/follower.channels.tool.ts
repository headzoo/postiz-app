...
export class FollowerChannelsTool implements AgentToolInterface {
  ...
  run() {
    return createTool({
      ...
      mcp: { annotations: { ...followerToolAnnotations, title: 'List follower channels' } },
      ...
      execute: async (inputData, context) => {
        ...
        const channels = await this._integrationService.getFollowerChannels(organization);
        return {
          output: channels.map((channel) => ({
            ...
            ...(safeHttpUrl(channel.picture) ? { picture: safeHttpUrl(channel.picture) } : {}),
            sorts: channel.sorts,
