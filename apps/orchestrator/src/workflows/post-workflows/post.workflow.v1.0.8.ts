...
export async function postWorkflowV108({
  ...
  for (let i = 0; i < postsList.length; i++) {
    ...
    // the publish - retrying after updatePost errors would duplicate the post
    let posted = false;
    let updated = false;
    ...
    for (const _ of iterate) {
      try {
        ...
        if (posted) {
          ...
          // already marked published, a failed updatePost shouldn't abort the
          // rest of the flow
          break;
        }
  ...
  while (list.length > 0) {
    ...
    if (todo.type === 'global') {
      for (const _ of iterate) {
        try {
          const process = await processPlugV107({
            ...todo,
          ...
          if (process) {
            const toDelete = list
              .reduce((all, current, index) => {
                if (
                  current.plugId === todo.plugId &&
                  current.source === todo.source
                ) {
                  all.push(index);
                }
